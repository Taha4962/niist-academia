...
const Profile = () => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      setError('')
      setSuccess('')
      const res = await api.put('/faculty/profile/me', formData)
      setProfile(res.data.profile)
      setEditing(false)
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      {/* Success Message */}
      {success && (
        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg text-sm transition-all duration-300 ease-in-out">
          ✅ {success}
        </div>
      )}
      ...
      {/* Error Message */}
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm transition-all duration-300 ease-in-out">
          ❌ {error}
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="flex items-center gap-5 mb-6 pb-6 border-b border-gray-100 relative">
          <div className="relative group">
            <div className="w-20 h-20 rounded-full bg-niist-navy flex items-center justify-center text-white text-2xl font-bold flex-shrink-0 overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <p className="font-mono text-gray-900 font-semibold mt-1">
              {profile?.employee_id}
            </p>
          </div>
          <div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between mb-3">
            ...
            {!editing ? (
              <button onClick={() => setEditing(true)} className="text-sm text-niist-blue hover:underline font-medium">
                ✏️ Edit
              </button>
            ) : (
              <div className="flex gap-2">
                <button onClick={() => { setEditing(false); setError(''); }} className="text-sm text-gray-500 hover:underline">
                  Cancel
                </button>
                <button onClick={handleUpdate} disabled={saving} className="text-sm bg-niist-navy text-white px-4 py-1.5 rounded-lg hover:bg-niist-blue disabled:opacity-50">
                  {saving ? 'Saving...' : 'Save'}
                </button>
              </div>
            )}
          ...
          <div>
            ...
            {editing ? (
              <input type="tel" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-niist-blue" placeholder="Enter phone number" />
            ) : (
          ...
          <div>
            ...
            {editing ? (
              <input type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-niist-blue" placeholder="Enter email" />
            ) : (
          ...
          {/* Not editable fields notice */}
          {editing && (
            <p className="text-xs text-gray-400 bg-gray-50 rounded-lg px-3 py-2 mt-4 inline-block">
              ℹ️ Name and designation can only be updated by HOD.
            </p>
          )}
        </div>
      </div>
