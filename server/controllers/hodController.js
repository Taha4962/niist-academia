...
const updateStudent = async (req, res) => {
  try {
    const { id } = req.params;
    const { cgpa, address, blood_group, profile_photo, admission_year } = req.body;
    
    const { rows } = await db.query(
      `UPDATE students SET cgpa=COALESCE($1, cgpa), address=COALESCE($2, address), 
       blood_group=COALESCE($3, blood_group), profile_photo=COALESCE($4, profile_photo), 
       admission_year=COALESCE($5, admission_year)
       WHERE student_id=$6 RETURNING *`,
      [cgpa, address, blood_group, profile_photo, admission_year, id]
    );
    if (rows.length === 0) return res.status(404).json({ message: 'Student not found' });
    res.json(rows[0]);
  } catch (err) {
